...
ratings_couples |>
  ...
  geom_line(aes(y = fit)) +
  geom_line(aes(y = cf), linetype = 2) ## add dashed line with counterfactual trajectory to plot
```
...
newgirl |> 
  dplyr::mutate(fit = its_mod$fitted.values,
                cf = its_mod$coefficients[1] + its_mod$coefficients[2] * eps_since_kiss ## counterfactual, extending pre-kiss model
                ) |>
  ggplot(aes(x = eps_since_kiss, y = rating, group = after_first_kiss)) +
  annotate("rect", xmin = 0, xmax = Inf, ymin = -Inf, ymax = Inf, alpha=0.2, fill = "magenta") +
  geom_point(aes(color = after_first_kiss), alpha = 0.5) +
  geom_line(aes(y = fit)) +
  geom_line(aes(y = cf), linetype = 2) + ## add dashed line with counterfactual trajectory to plot
  theme_barbie() +
  ...
  scale_color_manual(values = c("TRUE" = "blue", "FALSE" = "darkgray"), guide = "none")
```